...
const MIN_REQUEST_AMOUNT_BEFORE_KST = 5000;
const MIN_REQUEST_AMOUNT_AFTER_KST = 1000;
const MIN_REQUEST_AMOUNT_CHANGE_KST_MS = new Date('2025-11-14T00:00:00+09:00').getTime();

function getCurrentKstTimeMs() {
  const now = new Date();
  const utcMs = now.getTime() + now.getTimezoneOffset() * 60000;
  return utcMs + KST_OFFSET_MS;
}

function getMinRequestAmount() {
  const nowKstMs = getCurrentKstTimeMs();
  return nowKstMs >= MIN_REQUEST_AMOUNT_CHANGE_KST_MS
    ? MIN_REQUEST_AMOUNT_AFTER_KST
    : MIN_REQUEST_AMOUNT_BEFORE_KST;
}

function getMinAmountToastMessage(minAmount) {
  return minAmount === MIN_REQUEST_AMOUNT_AFTER_KST
    ? '천원 이상부터 지급신청 가능'
    : '5천원 이상부터 지급신청 가능';
}

// 토스트 재출력을 방지하기 위한 세션 상태 저장 객체
window.myPrizeState = window.myPrizeState || {};
window.myPrizeState.dismissedMismatchToast = window.myPrizeState.dismissedMismatchToast || false;
...
//			      webkit.messageHandlers.applycost();
	    ...
	    // 토스트 확인 상태를 설정하여 재비활성화 방지
	    window.myPrizeState = window.myPrizeState || {};
	    window.myPrizeState.dismissedMismatchToast = true;
	  ...
	  // 일반 브라우저에서 실행
	    ...
	    // 일반 브라우저에서도 토스트 확인 후 request-button 활성화
	    const reqBtn = document.querySelector('.request-button');
	    if (reqBtn) {
	      reqBtn.disabled = false;
	      reqBtn.classList.remove('clicked');
...
function showPrizeWarningToast(minAmount) {
	// 기존 경고 토스트가 있다면 제거
	const existing = document.querySelector('.prize-toast-warning');
	if (existing) existing.remove();
	const toast = document.createElement('div');
	toast.className = 'prize-toast-warning';
	const amount = typeof minAmount === 'number' ? minAmount : getMinRequestAmount();
	const toastMessage = getMinAmountToastMessage(amount);
	toast.innerHTML = `<span class="prize-toast-warning-text">${toastMessage}</span>`;
...
const reqBtn = document.querySelector('.request-button');
  ...
  // ───── 추가: 총액이 최소 신청 금액 미만이면 신청 불가 ─────
  const minRequestAmount = getMinRequestAmount();
  if ((userData.currentAmount || 0) < minRequestAmount) {
    showPrizeWarningToast(minRequestAmount);
    return;
  }
...
async function initMyPrizeSection() {
  ...
  try {
    ...
    if (prizeResponse.success && prizeResponse.prizeHistory) {
      ...
      userData.beforePoint = prizeResponse.beforePoint;
...
//      //console.log('MyPrize - Fallback 데이터 사용, 계산된 totalPages:', calculatedTotalPages);
    ...
    let showExpirePointInfo = false;
    let eventExpireDetail = '';
    let eventGoldenTitle = '';
    let eventGoldenDetail = '';
    try {
      ...
      if (eventInfoResponse.success && eventInfoResponse.eventInfo) {
        ...
        eventExpireDetail = eventInfo.eventExpireDetail;
        eventGoldenTitle = eventInfo.eventGoldenTitle;
        eventGoldenDetail = eventInfo.eventGoldenDetail; 
        if (eventResultEndDate) {
          ...
          showExpirePointInfo = userData.beforePoint > 0;
        } else {
        ...
        <div class="prize-group">
          ...
          <div style="display:flex;align-items:center;gap:4px;">
	        ...
	        <div class="expire-info">
	          <span class="expire-date">${eventExpireDetail}</span>
	        </div>
	        <div class="expire-amount-info">
	          <span class="expire-amount">${formatNumber(userData.beforePoint || 0)} 원</span>
	          <span class="expire-badge-text">소멸 예정</span>
	        </div>
	        ` : ''}
	        <div class="golden-time-info">
	          <h3 class="golden-time-title">${eventGoldenTitle}</h3>
	          <ul class="golden-time-details">
	            ${convertToLiTags(eventGoldenDetail)}
	          </ul>
	        </div>
    ...
    const historyItemsHtml = currentHistory.map(item => {
      ...
      if (item.history === '올킬 당첨') {
        ...
        sign = '';
      } else if (item.history === '상금 지급 완료' || item.history.includes('올킬 상금 소멸')) {
    ...
    // ───── 6) 페이지 접속 시 토스트 알림 표시 (세션 중 재출력 방지) ─────
	if (currentHistory.some(item => item.history === "지급 정보 불일치")) {
	    // 토스트를 확인한 경우가 아니면 버튼 비활성화
	    if (!window.myPrizeState || window.myPrizeState.dismissedMismatchToast !== true) {
...
function renderWithExistingData() {
  ...
  const minRequestAmount = getMinRequestAmount();
  const minRequestAmountFormatted = safeFormatNumber(minRequestAmount);
  ...
  const sectionHtml = `
    <div class="my-prize-container">
      ...
      <div class="prize-group">
        ...
        <div class="golden-time-info">
          <h3 class="golden-time-title">11월 골든타임 안내 (11/1 ~ 11/5)</h3>
          <ul class="golden-time-details">
            <li>10월 미신청 상금은 골든타임 기간에 획득한 상금과 합산하여 신청 가능</li>
            <li>합산된 총 금액이 ${minRequestAmountFormatted} 원 이상일 경우 가능</li>
            <li>'11월 6일 00시'에 지급되지 않은 10월 상금은 '자동 소멸'</li>
          </ul>
        </div>
        <button class="request-button" onclick="requestPrizePayment()">상금 지급 신청</button>
      </div><!-- /.prize-group -->
