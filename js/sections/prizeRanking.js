...
;(function(window, $) {
  // Prize Ranking Section 초기화
  function initPrizeRankingSection() {
    console.log("Initializing Prize Ranking Section");
    
    // 데이터 확인
    if (!window.rankingData || !window.utils) {
      console.error("Missing required data for Prize Ranking Section", { 
    ...
    const rankingData = window.rankingData;
    const state = window.appState || { activeTab: 'weekly' };
    const { formatNumber } = window.utils;
    
    // 대상 컨테이너
    const $container = $('#prize-ranking-section');
    if ($container.length === 0) {
      console.error('[initPrizeRankingSection] #prize-ranking-section not found');
      return;
    }
    ...
    console.log("Prize Ranking Section initialized successfully");
  }
