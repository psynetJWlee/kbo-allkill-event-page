...
function createWinnerDateNavigation(totalCount = 0, customMessage = null) {
  ...
  const prevArrow = hasPrevDate 
    ? `<div style="position: absolute; left: 5px; height: 120px; background-color: transparent; width: 65px; cursor: pointer; display: flex; align-items: center; justify-content: center;" onclick="winnersGoToPrevDate()"><div class="winner-nav-arrow prev"></div></div>`
    : '';
  
  // 다음 날짜 (더 최근) 버튼 - 인덱스가 감소하면 최근으로 이동
  const hasNextDate = currentWinnerDateIndex > 0;
  const nextArrow = hasNextDate
    ? `<div style="position: absolute; right: 5px; height: 120px; background-color: transparent; width: 65px; cursor: pointer; display: flex; align-items: center; justify-content: center;" onclick="winnersGoToNextDate()"><div class="winner-nav-arrow next"></div></div>`
    : '';
  ...
  return `
    <div class="w-full flex flex-col items-center relative" style="position: relative;">
      ${prevArrow}
      <div class="winners-title">
        <span class="title-text">${dateDisplay}</span>
        <div class="title-underline"></div>
        <p class="winners-count w-full text-center">
          ${message}
        </p>
...
function renderMinimalFallback($sec) {
  ...
  const sectionHtml = `
    <div class="w-full flex flex-col items-center relative">
      <h2>오늘의 당첨자</h2>
      <p class="winners-count w-full text-center">총 30명</p>
    </div>
