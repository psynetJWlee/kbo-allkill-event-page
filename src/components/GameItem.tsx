...
const GameItem: React.FC<GameItemProps> = ({
  ...
  return (
    <div
      ...
      data-index={id}
    >
      {/* Today 모드(showTime) & 첫 번째 경기(index 0)일 때만 동그라미 오버레이 */}
      {showTime && index === 0 && (
        <img
      ...
      <div className="team-column flex justify-start flex-none">
        ...
        <div
          className={`vote-count ${
            homeTeam.votes >= awayTeam.votes ? 'higher' : 'lower'
          }`}
        >
        ...
        {showTime ? (
          <>
            <div className="score-container flex items-center">
              <span
                className={`score text-[30px] ${
                  homeTeam.votes > awayTeam.votes ? 'text-[#FFC736]' : 'text-[#FFFFFF]'
                }`}
              >
                {homeTeam.votes}
              </span>
              ...
              <span
                className={`score text-[30px] ${
                  awayTeam.votes > homeTeam.votes ? 'text-[#FFC736]' : 'text-[#FFFFFF]'
                }`}
              >
                {awayTeam.votes}
              </span>
            </div>
            <span className="match-status text-[14px] text-[#FFFFFF] mt-1">
              {status}
            </span>
          </>
        ) : (
      ...
      <div className="team-column flex justify-end flex-none text-right">
        ...
        <div
          className={`vote-count ${
            awayTeam.votes >= homeTeam.votes ? 'higher' : 'lower'
          }`}
        >
